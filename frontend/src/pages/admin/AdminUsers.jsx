...
const AdminUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await api.get(`${API_URL}/api/admin/users`, {
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to delete the user? This action cannot be undo.",
      )
    )
      return;
    try {
      await api.delete(`${API_URL}/api/admin/users/${id}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
              ...
              {filteredUsers.length > 0 ? (
                    ...
                    <td className={s.tdActions}>
                      <div className={s.actionsWrapper}>
                        <button
                          ...
                          title={user.isBlocked ? "Unblocl User" : "Block User"}
                        >
                          {user.isBlocked ? (
                ...
                <tr>
                  <td className={s.emptyState} colSpan="5">
                    No Users found for this filters..
                  </td>
                </tr>
              )}
            </tbody>
