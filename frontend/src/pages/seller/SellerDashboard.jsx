...
const SellerDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        const [statsRes, propsRes, inqRes] = await Promise.all([
          api.get(`${API_URL}/api/property/seller/dashboard`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          api.get(`${API_URL}/api/property/my`, {
            headers: { Authorization: `Bearer ${token}` },
          }),

          api.get(`${API_URL}/api/inquiry/seller`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this listing?"))
      return;
    try {
      await api.delete(`${API_URL}/api/property/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const statCards = [
    ...
    {
      title: "Live Listings",
      value: stats.activeVisitings?.toLocaleString() || "0",
      ...
      <div className={s.widgetsGrid}>
        <div className={s.inquiriesWidget}>
          ...
          <div className={s.inquiriesList}>
            {inquiries.map((inq, i) => (
              <div key={inq._id || i} className={s.inquiryItem}>
          ...
          <div className={s.tipsList}>
            <div className={s.tipCardHighViews}>
              ...
              <p className={s.tipTextHighViews}>
                Your Visitings are trending. Try adding video tours to increase
                interest.
              </p>
            </div>
