...
const PropertyDetails = () => {
  ...
  //to fetch the property details

  useEffect(() => {
    const fetchDetails = async () => {
      try {
        setLoading(true);
        const res = await api.get(`${API_URL}/api/property/${id}`, {
        ...
        if (user && user.role === "buyer") {
          const wishRes = await api.get(`${API_URL}/api/wishlist`, {
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      if (isInWishlist) {
        await api.delete(`${API_URL}/api/wishlist/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleChatStart = async () => {
    ...
    if (!id || !property?.seller?._id) {
      return alert("Property or seller details missing.");
    }

    const sellerId =
      typeof property?.seller === "object"
        ? property?.seller?._id
        : property?.seller;
    ...
    try {
      ...
      console.error("Error statring in the chat", error.response?.data);
            ...
            <div className={s.descriptionSection}>
              ...
              <p className={s.descriptionText}>
                {property.description ||
                  "No description available for this propety.."}
              </p>
            </div>
