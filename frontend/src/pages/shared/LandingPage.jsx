...
const LandingPage = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const res = await api.get(`${API_URL}/api/property/counts`);
        if (res.data.success) {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const res = await api.get(`${API_URL}/api/property?city=${searchTerm}`);
        setProperties(res.data.properties || res.data || []);
        setError(null);
      } catch {
        setError("Failed to loadproperties. Please try again");
      } finally {
        setLoading(false);
      }
    };
      ...
      <section className={s.heroSection}>
        <div className={s.heroContent}>
          ...
          <p className={s.heroSubtitle}>
            ...
            verified listings, correct with top agents, and find a place you'll
            love.
          </p>
          ...
          <div className={s.footerMainGrid}>
            <div className={s.footerBrand}>
              ...
              <div className={s.socialIcons}>
                {[FaFacebookF, FaXTwitter, FaInstagram, FaLinkedin].map(
                  (Icon, idx) => (
